...
async function apiConsumerSets(name) {
    await fetch(` https://omgvamp-hearthstone-v1.p.rapidapi.com/cards/${name}`, {
            "method": "GET",
            "headers": {
    ...
    for (const key in obj) {
        if (obj.hasOwnProperty(key)) {
            ...
            if (key == "card_text" && obj[key] != "") {
                descricao2 = `<p  class="text-center font-mono">${obj.card_text}</p>`;
