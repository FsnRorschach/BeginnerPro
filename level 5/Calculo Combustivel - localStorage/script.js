...
let var_dias_semana;
let msgvazia;
let array_card_modal = JSON.parse(localStorage.getItem('combustivel')) || [];
...
document.querySelector('form button').addEventListener('click', (evt) => {
    ...
    if (msgvazia != true) {
        ...
        let gast_men = gasto_mensal(var_quilometros_dia, var_media, var_preco, var_dias_semana);
        let gast_anu = gasto_anual(var_quilometros_dia, var_media, var_preco, var_dias_semana);
...
let gasto_mensal = (km_dia, km_med_lit, preco_lit, qtd_dias_sem) => {
    return parseFloat(parseFloat(((preco_lit / km_med_lit) * km_dia) * (qtd_dias_sem * 4)).toFixed(2));
};

// fucntion para calcular o gasto anual
let gasto_anual = (km_dia, km_med_lit, preco_lit, qtd_dias_sem) => {
    return parseFloat(parseFloat(((preco_lit / km_med_lit) * km_dia) * (qtd_dias_sem * 48)).toFixed(2));
};
...
document.getElementById('salvar').addEventListener('click', () => {
    ...
    descarta_modal();
    console.log(obj);
    array_card_modal.push(obj);
    console.log(array_card_modal);
    localStorage.setItem('combustivel', JSON.stringify(array_card_modal));
    setar_modal_historico();
});

// function de seta no historico
let setar_modal_historico = () => {
    array_card_modal = JSON.parse(localStorage.getItem('combustivel'));
    mostrar_modal_historico();
    const div_modal_hist = document.querySelector('#historico div table tbody');
    deletaHistorico();
    array_card_modal.forEach((element, index) => {
        ...
        tr_modal_hist.innerHTML += `
                                <td id="data">${array_card_modal[index].data}</td>
                                <td id="preco_comb">${array_card_modal[index].val_preco_gas}</td>
                                <td id="media_km-lit">${array_card_modal[index].val_media_car}</td>
                                <td id="dia">${array_card_modal[index].gasto_diario}</td>
                                <td id="sem">${array_card_modal[index].gasto_semanal}</td>
                                <td id="mes">${array_card_modal[index].gasto_mensal}</td>
                                <td id="ano">${array_card_modal[index].gasto_anual}</td>
...
document.getElementById('fundo-modal').addEventListener('click', function (e) {
